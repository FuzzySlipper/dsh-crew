...
function CrewWorkbenchInbox({ loading, error, values }) { return _jsxs("section", { className: "dshCrewSessionPrompt dshCrewWorkbenchInbox", children: [_jsx("strong", { children: "Workbench inbox" }), loading ? _jsx("p", { className: "dshCrewSessionsMuted", children: "Loading replies\u2026" }) : error === undefined ? values.length === 0 ? _jsx("p", { className: "dshCrewSessionsMuted", children: "No messages have arrived for this workbench." }) : values.map(value => _jsxs("article", { className: "dshCrewSessionEvent", children: [_jsxs("header", { children: [_jsx("strong", { children: value.sender }), _jsx("time", { dateTime: value.createdAt, children: value.createdAt })] }), _jsx("p", { children: value.body }), _jsxs("small", { children: [value.state, value.replyToMessageId === undefined ? '' : ` · reply to ${value.replyToMessageId}`] })] }, value.deliveryId)) : _jsx("p", { className: "dshCrewSessionsError", role: "status", children: error })] }); }
