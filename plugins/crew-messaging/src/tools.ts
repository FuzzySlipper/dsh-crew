...
export function installScopedTools(ctx: Context, service: CrewMessagingService): () => void {
  ...
  const install = (agent: Agent): void => {
    ...
    try {
      disposers.push(agent.ctx.systemPrompt.section({ name: 'crew-messaging:policy', order: 65, text: () => 'Use crew_message to send a durable text message to a configured fabric address. An ordinary delivered crew message explains how to send a linked reply when one is warranted. A delivered reply acknowledges prior work and must not be answered merely because it is a reply.' }))
